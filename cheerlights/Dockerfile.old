...
RUN apt-get update && \
    apt-get install -qy \
     --no-install-recommends build-essential wiringpi git curl ca-certificates && \
...
RUN go build -o /usr/bin/cheerlights

FROM resin/rpi-raspbian
WORKDIR /root/
COPY --from=0 /usr/bin/cheerlights /root/cheerlights
